...
def create_app():
    ...
    UPLOADS_DIR = os.path.join(PROJECT_DIR, 'storage', 'uploads')
    REPORTS_DIR = os.path.join(PROJECT_DIR, 'storage', 'reports')
    SCREENSHOTS_DIR = os.path.join(PROJECT_DIR, 'storage', 'screenshots')
    ...
    # Initialize MongoDB
    mongo = PyMongo(app)
    print(f"MongoDB initialized: {mongo}")
    ...
    def login():
        ...
        return jsonify({"access_token": access_token, "username": user.get('username')}), 200
        ...
        for run in test_runs:
            if run.get('report_path') and os.path.exists(run['report_path']):
                with open(run['report_path']) as f:
                    ...
                    # adapt summary keys if old format
                    summary = report_data.get('summary', {})
                    # rename total_tests to total for frontend convenience
                    if 'total_tests' in summary:
                        summary['total'] = summary.pop('total_tests')
                    data.append({
    ...
    def manual_test():
        ...
        result = mongo.db.manual_tests.insert_one(manual_test)
        return jsonify({"message": "Manual test saved", "_id": str(result.inserted_id)}), 201

    @app.route("/api/manual_tests", methods=["GET"])
    @jwt_required()
    def get_manual_tests():
        user_id = get_jwt_identity()
        tests = list(mongo.db.manual_tests.find({'user_id': user_id}).sort('created_at', -1))
        for test in tests:
            test['_id'] = str(test['_id'])
        return jsonify(tests), 200

    @app.route("/api/manual_test/<test_id>", methods=["DELETE"])
    @jwt_required()
    def delete_manual_test(test_id):
        user_id = get_jwt_identity()
        result = mongo.db.manual_tests.delete_one({'_id': ObjectId(test_id), 'user_id': user_id})
        if result.deleted_count == 0:
            return jsonify({"message": "Test not found"}), 404
        return jsonify({"message": "Manual test deleted"}), 200

    @app.route("/api/manual_test/<test_id>", methods=["PATCH"])
    @jwt_required()
    def update_manual_test(test_id):
        user_id = get_jwt_identity()
        data = request.get_json()
        status = data.get('status')
        if status not in ['PENDING', 'PASS', 'FAIL']:
            return jsonify({"message": "Invalid status"}), 400
        result = mongo.db.manual_tests.update_one(
